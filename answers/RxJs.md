...
| :--- | :--- | :--- |
| **`concatMap`** | Ждет завершения и сохраняет очередь. | Строгая последовательность (заказы, транзакции). |
| **`switchMap`** | Отменяет текущий запрос, если пришел новый. | Живой поиск, навигация. |
| **`exhaustMap`** | Игнорирует новые события, пока не завершен текущий запрос. | Кнопки отправки форм (защита от двойного клика). |
| **`mergeMap`** | Выполняет все запросы одновременно (параллельно). | Когда порядок не важен и результат нужен максимально быстро. |
